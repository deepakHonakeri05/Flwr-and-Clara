...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m nvflare.private.fed.app.client.client_train -m $DIR/.. -s fed_client.json --set secure_train=true uid=site1 config_folder=config 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl | (tee -a $DIR/../log.txt & echo $! >&4 ) 4>$DIR/../tee.fl )
  pid=`cat $DIR/../pid.fl`
  echo "new pid ${pid}"
}
