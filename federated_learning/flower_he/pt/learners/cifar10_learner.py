...
class CIFAR10Learner(Learner):  # also supports CIFAR10ScaffoldLearner
    ...
    def initialize(self, parts: dict, fl_ctx: FLContext):
        ...
        self.TRAIN_DATA_PATH = "/workspace/data/flowers_dataset/train1/"
        self.TEST_DATA_PATH = "/workspace/data/flowers_dataset/test/"
        self.BATCH_SIZE = 32

        # Set dataset
        self.train_dataset = torchvision.datasets.ImageFolder(root=self.TRAIN_DATA_PATH, transform=self.transform_train)
        
        self.train_loader = data.DataLoader(self.train_dataset, batch_size=self.BATCH_SIZE, shuffle=True,  num_workers=4)

        
        self.valid_dataset = torchvision.datasets.ImageFolder(root=self.TEST_DATA_PATH, transform=self.transform_valid)
        self.valid_loader = data.DataLoader(self.valid_dataset, batch_size=self.BATCH_SIZE, shuffle=True,  num_workers=4)
    ...
    def local_train(self, fl_ctx, train_loader, model_global, abort_signal: Signal, val_freq: int = 0):
        for epoch in range(self.aggregation_epochs):
            ...
            for step, (x, y) in enumerate(train_loader):
                ...
                b_y = F.one_hot(y.to(self.device), num_classes=13)
                # zero the parameter gradients
                self.optimizer.zero_grad()
                # forward + backward + optimize
                outputs = self.model(b_x)
                loss = self.criterion(outputs, b_y.float())
                ...
                self.optimizer.step()
                total += b_y.size(0)
                correct += ( (torch.max(outputs.data.to(self.device), 1)[1]).to(self.device) == y.to(self.device)).sum().item()
                print("epoch ", epoch, ",", "loss: ", loss.item()," Training Accuracy : ", (correct / total *100),"%")
                current_step = epoch_len * self.epoch_global + step
    ...
    def local_valid(self, valid_loader, abort_signal: Signal, tb_id=None, fl_ctx=None):
        ...
        with torch.no_grad():
            ...
            print("Validation Accuracy : ",metric)
            if tb_id:
